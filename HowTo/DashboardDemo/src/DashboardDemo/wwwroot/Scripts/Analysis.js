...
function init() {
    ...
    // init the layout definition.
    nsSplitLayout.definition = wsSplitLayout.definition;
    nsSplitLayout.orientation = 1;
    var tiles = [];
    var checkSplitTile = function (item) {
        if (item instanceof c1.nav.split.SplitTile) {
            item.size = null;
            tiles.push(item);
        } else if (item instanceof c1.nav.split.SplitGroup) {
            if (item.children) {
                item.children.forEach(function (child) {
                    checkSplitTile(child);
                });
            }
        }
    }
    nsSplitLayout.items.forEach(function (item) {
        checkSplitTile(item);
    });
    nsSplitLayout.items.clear();
    tiles.forEach(function (tile) {
        nsSplitLayout.items.push(tile);
    });
}

function relayout() {
    var eleIds = ['chartRegionSale', 'chartOpportunities'];
