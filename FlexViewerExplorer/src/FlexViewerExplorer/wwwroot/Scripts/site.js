...
function loadArReport(reportPath) {
    loadReport(arServiceUrl, reportPath, '', false)
}

function loadSsrsReport(reportPath) {
    loadReport(c1WebApiServiceUrl, reportPath, '', false);
}

function loadReport(serviceUrl, filePath, reportName, paginated) {
    ...
    reportViewer.paginated = paginated;
}
