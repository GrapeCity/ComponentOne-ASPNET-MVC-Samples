    ...
    public static class ReportFiles
    {
        ...
        private static void EnsureInit()
        {
            lock (_locker)
            {
                ...
                var reportInfos = XElement.Load(HttpContext.Current.Server.MapPath("~\\Content\\ReportInfos.xml"));
                ...
                var currentCulture = System.Threading.Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName.ToLower();

                foreach (var category in reportInfos.Elements("Category"))
                {
                    // check the supported culture
                    var cultureAttr = category.Attribute("Culture");
                    var culture = cultureAttr == null ? "" : cultureAttr.Value.ToLower();
                    if (!string.IsNullOrEmpty(culture) && !culture.Contains(currentCulture))
                    {
                        continue;
                    }
